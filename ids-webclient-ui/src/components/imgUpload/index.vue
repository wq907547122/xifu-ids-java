...
<template>
  <div class="img-upload-parent">
    ...
    <el-upload v-if="!isShowImgOnly"
               ...
               :before-upload="beforeAvatarUpload" title="点击选择上传图片">
    ...
    <span v-if="isShowLoading && isLoading"><i class="el-icon-loading"></i>上传中...</span>
  </div>
</template>
...
let props = {
  ...
  defaultWH: { // 如果需要验证宽高的宽和高
    type: Array,
    default: function () {
      return [40, 40, 0, 0] // 第一个是宽  第二个是高, 第三个参数：宽的误差，第四个参数：高的误差
    }
  }
}
export default {
  ...
  created () {
    ...
    if (this.uploadUrl) { // 上传的url路径
      ...
      this.action = '/fileManager/upload'
    }
    if (this.downloadUrl) { // 回显图片获取图片信息的url路径
      this.fileDownPath = this.downloadUrl
    } else {
      this.fileDownPath = '/fileManager/downloadFile'
    }
  },
  components: {
  },
  ...
  methods: {
    ...
    beforeAvatarUpload (file) { // 上传之前的验证
      ...
      if (this.isValidWH) { // 如果还需要验证宽高
        ...
        if (confirm('上传LOGO是要求40*40，确认上传？')) {
          return true
        } else {
          return false
        }
    ...
    handleAvatarSuccess (res, file) {
      // 通知上级修改fileId
      if (res) {
        this.$emit('update:fileId', res)
        this.$emit('on-upload-success', res, file) // 上传成功的回调函数
        this.time = Date.parse() // 更新时间
      } else {
        this.$message.error('上传图片失败')
        this.$emit('on-upload-error', res) // 上传失败的回调函数
      }
