  ...
  <el-menu mode="vertical" router unique-opened :default-active="$route.path"
    ...
    <template v-if="subMenuState">
      <template v-for="item in menus"
        ...
        <el-submenu v-bind:key="item.name" v-if='!item.noDropdown' :index="item.name">
          <template slot="title">
            <i class="el-icon-nav" :class='item.icon'></i>
            <span slot="title">{{$t(item.name)}}</span>
          </template>
          <template v-for="child in item.children" v-if='!child.hidden'>
            <el-menu-item v-bind:key="child.path" :index="item.path + '/' + child.path">
              <span slot="title">{{$t(child.name)}}</span>
            </el-menu-item>
          </template>
        </el-submenu>
        <el-menu-item v-bind:key="item.path" v-else
    ...
    <template v-else>
      <template v-for="item in menus" v-if="!item.hidden">
        <el-menu-item v-bind:key="item.path" :route="{path: item.path}" :index="item.path">
          <i class='el-icon-nav' :class='item.icon'></i>
          <span slot="title">{{$t(item.name)}}</span>
        </el-menu-item>
      </template>
