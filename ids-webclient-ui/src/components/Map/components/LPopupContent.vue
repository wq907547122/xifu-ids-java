  ...
  <div>
      ...
      <div class="popup-custom-station-container">
        ...
        <div class="popup-box">
          ...
          <div class="popup-station-description">
            <p class="popup-box-title">{{$t('modules.exhibition.stationInfo.description')}}</p>
    ...
    <template v-else-if="type === 'users'">
      <div class="popup-custom-user-container">
        <div class="popup-user-title">{{data.userName}}缺陷列表(共{{data.list && data.list.length}}个)</div>
        <!-- 缺陷列表的查询 -->
        <el-table :data="data.list" style="width: 100%;margin-top: 10px;" border maxHeight="374">
          ...
          <el-table-column prop="taskStatus" label="任务状态" align="center">
            <template slot-scope="scope">
              <div class="s" :title="scope.row.taskStatus | taskStatusFormater">
                <label :class="['defect-color-state' + scope.row.taskStatus]">{{scope.row.taskStatus | taskStatusFormater}}</label>
              </div>
            </template>
          </el-table-column>
        </el-table>
