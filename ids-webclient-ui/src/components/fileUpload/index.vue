<template>
    <div class="file-upload-parent">
      <el-upload
        ...
        <div v-if="isShowTip" slot="tip" class="el-upload__tip">请选择文件</div>
...
export default {
  ...
  methods: {
    beforeUpload (file) { // 上传文件之前的验证, 目前值验证大小
      ...
      if (isSuccess) {
        this.$emit('on-validate-success', file) // 当验证通过的时候的回调函数
    ...
    handleUploadSuccess (res, file) { // 文件上传成功后的事件
      if (!res) { // 上传成功
        ...
        this.$message.error(res.message || this.$t('components.fileUpload.uploadFail'))
    ...
    handlUploadFailed (err1, file, fileList) { // 失败后就不显示对话框
      this.isLoading = false // 完成后就不使用等待框了
      this.$message.error(JSON.parse(err1.message).message || '上传失败')
      console.warn('上传失败', err1)
    }
  },
  watch: {},
  computed: {
