...
public class ExcelUtil {
    ...
    private Integer sheetNum = 1; // sheet页的数量
    /**
     * 注册url
     */
     ...
     * @throws IOException
     */
    public ExcelUtil(MultipartFile file) throws IOException {
        String originalFilename = file.getOriginalFilename();
        if (!isExcel(originalFilename)) { // 不是excel文件
            throw new RuntimeException("File is not excel file");
        }
...
//        return pathname.endsWith(".xls") || pathname.endsWith(".xlsx");
        return StringUtils.endsWith(pathname, SUFFIX_2003) || StringUtils.endsWith(pathname, SUFFIX_2007);
    }
    ...
    /**
     * 读取指定sheet也的指定行开始的数据
     * @param sheetIx sheet也的下标 从0开始
     * @param startRow 开始读取的行 从0开始
     * @return
     * @throws Exception
     */
    public List<List<String>> read(int sheetIx, int startRow) throws Exception {
        return read(sheetIx, startRow, getRowCount(sheetIx) - 1);
    }
